...
set BIN=""
if EXIST bin\nul set BIN=bin\
rem if EXIST %BIN%mintty.exe goto startmin
...
rem Blank the path, to keep things inside short and sweet
rem
set PATH=

start %BIN%mintty.exe --icon git.ico --title mintty -o Font=Consolas -o FontHeight=11 -o AllowBlinking=yes -w max -e /bin/bash --login -i
exit
